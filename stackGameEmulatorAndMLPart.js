    ...
    var sketchProc = function(processingInstance) {
     with (processingInstance) {
        size(400, 400);
        frameRate(30);
        var machineActDistance = random(-1, 1);
        var trials = 0;
        ...
        if (dist(stackX,stackY,centerX,centerY) <= convertedDistance || dist(stackX - stackV*stackMoves[stackMove],stackY - stackV*stackMoves[stackMove + 1],centerX,centerY) <= convertedDistance + abs(stackV)/2){
            cut();
        }
        };
